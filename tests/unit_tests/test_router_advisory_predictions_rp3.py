...
def _record() -> dict:
    return {
        ...
        "must_include_agents": ["a03_macro_policy"],
        "critical_agents": ["a03_macro_policy"],
        "nice_to_have_agents": [],
        "should_not_include_agents": ["a05_product_pricing"],
        "expected_layers": {"L2": ["a03_macro_policy"], "L3": []},
...
def test_provided_artifact_source_uses_reliability_shadow_not_manual_labels(
        ...
        json.dumps(
            {
                ...
                "route_reliability": {
                    ...
                    "cards": [
                        {
                            "agent_id": "a19_market_risk",
                            "priority": "strong",
                            "reason_codes": ["semantic:high", "priority:strong"],
                        }
                    ],
                    "groups": {
                        "strongly_recommended": ["a19_market_risk"],
                        "candidate": [],
                        "wildcard": [],
                        "deprioritized": [],
                    },
    ...
    assert prediction["advisory_agent_ids"] == ["a19_market_risk"]
    assert prediction["advisory_reason_codes"] == {
        "a19_market_risk": ["semantic:high", "priority:strong"]
    }
    assert "a03_macro_policy" not in prediction["advisory_agent_ids"]
    assert '"L3": ["a19_market_risk"]' in prompt
    assert '"L2": ["a03_macro_policy"]' not in prompt
...
def test_provided_artifact_wildcard_only_without_domain_card_noop(tmp_path) -> None:
        ...
        json.dumps(
            {
                ...
                "enabled": True,
                "retrieval_reason": "ok",
                "top_ranked_ids": ["a15_research_synthesis"],
                "route_reliability": {
                    "schema_version": "route_reliability_shadow_v0",
                    "confidence_band": "normal",
                    "low_confidence_fallback": False,
                    "cards": [
                        {
                            "agent_id": "a15_research_synthesis",
                            "priority": "hidden",
                            "wildcard_flag": True,
    ...
    assert context["advisory_agent_ids"] == []
    assert context["advisory_secondary_agent_ids"] == ["a15_research_synthesis"]
    assert prompt == "baseline"
...
def test_provided_artifact_wildcard_only_uses_non_wildcard_fallback(tmp_path) -> None:
    ...
    assert context["advisory_agent_ids"] == ["a04_industry_layout", "a03_macro_policy"]
    assert context["advisory_secondary_agent_ids"] == ["a15_research_synthesis"]
    assert context["advisory_agent_ids"] != ["a15_research_synthesis"]
    assert '"L2": ["a04_industry_layout", "a03_macro_policy"]' in prompt
    assert "secondary_agents_by_layer" in prompt
    assert "a15_research_synthesis" in prompt
...
def test_regression_like_wildcard_only_artifacts_include_domain_cards(tmp_path) -> None:
    cases = [
        (
            ...
            ["a04_industry_layout", "a10_industry_sentiment", "a15_research_synthesis"],
            ["a04_industry_layout"],
        ),
        (
            ...
            ["a03_macro_policy", "a04_industry_layout"],
        ),
        (
            ...
            ["a05_product_pricing", "a04_industry_layout"],
        ),
    ]
    records = []
                    ...
                    "cards": [
                        {
                            "agent_id": agent_id,
                            "priority": "hidden",
                            "wildcard_flag": agent_id == "a15_research_synthesis",
                            "reason_codes": ["guardrail:wildcard"]
                            if agent_id == "a15_research_synthesis"
...
def test_run_writes_provided_artifact_predictions_and_summary(tmp_path) -> None:
    ...
    assert payload["advisory_agent_ids"] == ["a19_market_risk"]
    assert payload["advisory_reason_codes"] == {"a19_market_risk": ["semantic:high"]}
