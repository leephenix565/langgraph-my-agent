...
def test_profile_card_loader_applies_valid_card_and_fallbacks(tmp_path: Path) -> None:
    ...
    cards_dir.mkdir()
    _write_card(
        cards_dir / "a03_macro_policy.json",
        {
    ...
    assert result.summary()["loaded_agent_ids"] == ["a03_macro_policy"]
    assert "positive_examples: Fed path and policy transmission" in registry["a03_macro_policy"].profile_text
    assert "profile_version: rp2-test" in registry["a03_macro_policy"].profile_text
    assert registry["a21_reg_compliance"].profile_text == (
        "description: Regulatory compliance review.\n"
        "capabilities: compliance\n"
        "input_type: compliance\n"
        "team: compliance"
...
def test_profile_card_loader_ignores_invalid_unknown_and_special_cards(tmp_path: Path) -> None:
    ...
    assert list(registry) == ["a03_macro_policy", "a21_reg_compliance"]
