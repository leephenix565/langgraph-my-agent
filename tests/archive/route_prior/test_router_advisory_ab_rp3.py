    ...
    return label_case_from_record(
        {
            ...
            "must_include_agents": ["a03_macro_industry_research", "a04_commodity_hedging"],
            "critical_agents": ["a04_commodity_hedging"],
            "nice_to_have_agents": [],
    ...
    return label_case_from_record(
        {
            ...
            "label_source": "manual_gold",
            "quality_conclusion_allowed": True,
            "must_include_agents": ["a19_risk_identification"],
            "critical_agents": ["a19_risk_identification"],
            "nice_to_have_agents": ["a10_stock_technical_analysis"],
            "should_not_include_agents": ["a17_traditional_valuation", "a06_financial_statement_analysis"],
        }
    )
...
def test_ab_comparison_metrics_cover_parser_and_label_deltas() -> None:
    case = _case()
    baseline = parse_side(_raw(["a03_macro_industry_research"]), AGENT_CATALOG)
    advisory = parse_side(
        _raw(["a03_macro_industry_research", "a04_commodity_hedging", "a05_annual_report_analysis"]),
        AGENT_CATALOG,
    )
...
def test_case_record_uses_supplied_predictions_without_live_model() -> None:
    case = _case()
    prediction = {
        "baseline_raw": _raw(["a03_macro_industry_research"]),
        "advisory_raw": _raw(["a03_macro_industry_research", "a04_commodity_hedging"]),
    }
...
def test_noop_prediction_does_not_count_advisory_induced_regression() -> None:
    ...
    record = build_case_record(
        ...
        prediction={
            ...
            "baseline": {
                "raw": _raw(["a03_macro_industry_research", "a04_commodity_hedging"]),
                "prompt_chars": 100,
                "token_count": 10,
                "latency_ms": 20.0,
            },
            "advisory": {
                "raw": _raw(["a03_macro_industry_research"]),
                "prompt_chars": 100,
                "token_count": 10,
                "latency_ms": 20.0,
            },
        ...
        json.dumps(
            {
                ...
                "advisory_noop_reason": "noop_no_advisory",
                "baseline": {"raw": _raw(["a03_macro_industry_research", "a04_commodity_hedging"])},
                "advisory": {"raw": _raw(["a03_macro_industry_research"])},
            },
        )
        + "\n",
        encoding="utf-8",
...
def test_legacy_prediction_artifact_schema_still_loads(tmp_path) -> None:
    ...
    assert "a04_commodity_hedging" in record["advisory"]["selected_agents"]
        ...
        json.dumps(
            {
                ...
                "id": "ab-case",
                "baseline": {
                    "raw": json.loads(_raw(["a03_macro_industry_research"])),
                ...
                "advisory": {
                    "raw": json.loads(
                        _raw(["a03_macro_industry_research", "a04_commodity_hedging"])
                    ),
...
def test_missing_prediction_metadata_yields_null_token_and_latency_deltas() -> None:
    record = build_case_record(
        ...
        prediction={
            "schema_version": "router_advisory_prediction_v0",
            "baseline": {"raw": _raw(["a03_macro_industry_research"])},
            "advisory": {"raw": _raw(["a03_macro_industry_research", "a04_commodity_hedging"])},
        },
    )
...
def test_case_record_uses_expected_layers_stub_without_prediction() -> None:
    ...
    record = build_case_record(
        case,
        agent_catalog=AGENT_CATALOG,
        expected_layers={"L2": ["a03_macro_industry_research"], "L3": ["a19_risk_identification"]},
    )
...
def test_label_stub_advisory_groups_agents_by_expected_layer() -> None:
    ...
    advisory_prompt = render_advisory_prompt(
        ...
        case,
        agent_catalog=AGENT_CATALOG,
        expected_layers={"L2": [], "L3": ["a19_risk_identification"]},
    )
    advisory = _advisory_payload(advisory_prompt)

    assert advisory["critical_agents_by_layer"] == {"L3": ["a19_risk_identification"]}
    assert advisory["must_include_agents_by_layer"] == {"L3": ["a19_risk_identification"]}
    assert advisory["nice_to_have_agents_by_layer"] == {
        "L2": ["a10_stock_technical_analysis"]
    }
    assert "a19_risk_identification" not in advisory["critical_agents_by_layer"].get("L2", [])
...
def test_case_record_uses_layer_aware_advisory_prompt_without_persisting_prompt_body() -> None:
    ...
    assert "a19_risk_identification" in record["advisory"]["selected_agents"]
