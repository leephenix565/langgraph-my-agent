...
Agent Catalog v2 now separates graph runtime layers from business taxonomy. `/sdb/dlut/agent_layer_latest.xlsx` is the latest business-layer/category authority, while `/sdb/dlut/智能体分工及访问接口.csv` plus `/sdb/dlut/智能体的描述.csv` remain endpoint/profile lineage. Runtime `layer` still means graph dispatch (`L1`/`L2`/`L3`/`L4`), not latest-sheet business layers such as `解析层` / `分析层` / `应用层` / `报告层`. The active runtime catalog is 25 enabled roles: 23 functional agents plus 2 special roles, with enabled layer counts L1=1, L2=12, L3=11, L4=1. `config/agents` contains 27 metadata files because `a05_annual_report_analysis` and `a21_portfolio_manager` are retained disabled as historical functional metadata. `a02_task_router` metadata remains gone, and the true Router runtime remains `router_node`. Metadata now carries `business_layer`, `business_category`, and `business_subcategory` for taxonomy alignment without changing State/public API schemas. `a03_macro_industry_research` remains enabled and callable as a live-verified macro wrapper even though it is not listed in the latest layer sheet; it is marked as a retained old-CSV value-analysis item pending an explicit taxonomy decision. The 13 generic external HTTP wrappers remain registration-level runtime integrations; registration is not live verification for every service.
