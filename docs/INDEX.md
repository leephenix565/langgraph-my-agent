...
| `docs/SYNC_OPS_1R2_RECURSIVE_COVERAGE_REPAIR.md` | SYNC-OPS-1R2 P2S recursive source coverage repair: source/support subroots, historical parity ledger, current prod coverage ledger, projection digest, temp reconstruction, and non-claims. |
| `docs/SYNC_OPS_2A_P2S_WRITER_DRY_RUN.md` | SYNC-OPS-2A temp-only P2S writer hardening: machine approval, environment snapshot, locks, artifact store, stage/verify/activate/rollback primitives, recovery journal, validation profiles, and non-claims. |
| `docs/SYNC_OPS_2A_R1_SOURCE_POLICY_AND_FULL_SCALE_REHEARSAL.md` | SYNC-OPS-2A-R1 P2S source-policy closure and full-scale temp writer rehearsal: role-based source selection, runtime asset manifest, unknown-file handling, full real-plan temp stage/verify/activate/rollback, and non-claims. |
| `docs/SYNC_OPS_2A_R2_EXECUTABLE_PLAN_AND_STAGED_APPROVAL.md` | SYNC-OPS-2A-R2 executable P2S plan contract freeze: removal of read-only/skeleton markers, stage-only approval request, activation template boundary, artifact-store preflight, rollback contract, summary consistency, and non-claims. |
...
| `docs/SYNC_OPS_2B0_R1_DURABLE_ARTIFACT_STORE_BOOTSTRAP.md` | SYNC-OPS-2B0-R1 real durable artifact-store bootstrap: exact stable-binding approval, created directory layout, `STORE_METADATA.json`, verification, P2S replan, and non-claims. |
| `docs/SYNC_OPS_2B1_REAL_P2S_STAGE_VERIFY.md` | SYNC-OPS-2B1 real P2S stage/verify: exact stage-only approval, durable run id, versioned baseline stage path, digest/secret/compile verification, active sandbox proof, activation request, and non-claims. |
| `docs/SYNC_OPS_2B2X_P2S_ACTIVATION_ROLLBACK_CLOSEOUT.md` | SYNC-OPS-2B2X P2S activation/rollback/reactivation closeout: first activation, rollback proof, final reactivation, old archive preservation, immutable stage proof, and topic closure. |
| `docs/SYNC_OPS_3X_SANDBOX_TO_PROD_AUTOMATION.md` | SYNC-OPS-3X sandbox-to-prod automation: experiment manifest, B/S/P/D diff, change-unit gate, approval and transaction writer contracts, backup/apply/rollback, fake process/live gates, historical replay, real no-op rehearsal, and SYNC-OPS-4X entry. |
| `docs/SYNC_OPS_4X_PUBLISH_AND_REBASE_CYCLE.md` | SYNC-OPS-4X publish-and-rebase cycle: 3X integration repairs, typed digest descriptors, projected prod after-state, precomputed P2S plan, cycle approval bundle, no-op cycle evidence, and topic closure. |
| `docs/SYNC_OPS_5A_FIRST_NONZERO_CYCLE_QUALIFICATION.md` | SYNC-OPS-5A first non-zero qualification: risk financial fraud mapping closure, baseline repair gate, multi-transaction/shared-member rehearsal, strict compensation, candidate selection, and 5B entry. |
| `docs/SYNC_OPS_5A_R1X_SOURCE_AUTHORITY_AND_FINAL_FREEZE.md` | SYNC-OPS-5A-R1X source-authority closure: old repair supersession, risk-fraud prod source recovery, full-baseline P2S rebase contract, candidate requalification, compound approval request, and non-claims. |
| `docs/SYNC_OPS_5A_R2X_SOURCE_LOSS_RECOVERY_AND_FINAL_FREEZE.md` | SYNC-OPS-5A-R2X source-loss closure: irreversible source-loss semantics, sibling candidate/shadow canary contract, full P2S rebase manifest, launch-authority blocker, candidate reselection, and non-claims. |
| `docs/SYNC_OPS_5A_R3X_APPROVED_LAUNCH_AUTHORITY.md` | SYNC-OPS-5A-R3X final freeze: 67/67 source provenance, approved sync-ops supervised launcher, recovery V3, full P2S V3, first-cycle V3, compound V3 request, and non-claims. |
| `docs/SYNC_OPS_5A_R4X_REAL_SHADOW_CANARY_AND_APPROVAL_CHAIN.md` | SYNC-OPS-5A-R4X real canary qualification: environment profile, incumbent/canary equivalence, canary-only approval, Recovery V4, P2S V4 staged boundary, conditional approval chain, and non-claims. |
| `docs/SYNC_OPS_5A_R5X_SOURCE_LOSS_CUTOVER_CONTRACT.md` | SYNC-OPS-5A-R5X cutover contract freeze: V4 supersession, fresh candidate requirement, full-tree descriptors, production launch authority, exact action ids, roll-forward state machine, V5 approval request, and non-claims. |
| `docs/SYNC_OPS_5A_R6X_CLEAN_CUTOVER_CANDIDATE.md` | SYNC-OPS-5A-R6X clean candidate hardening: V5 projection rejection, 67-action clean pre-start descriptor, action type/mode/executable binding, non-mutating offline validation, post-start runtime artifact policy, archive portability, and non-claims. |
| `docs/SYNC_OPS_5A_R7X_CUTOVER_TREE_DIGEST_PARITY.md` | SYNC-OPS-5A-R7X cutover tree digest parity: V6 projection/materialization mismatch, physical/classification digest split, directory mode action ledger, exact action semantics rehearsal, V7 approval request, and non-claims. |
| `docs/SYNC_OPS_5B_X_SOURCE_LOSS_AND_P2S_CLOSEOUT.md` | SYNC-OPS-5B-X upstream closeout: repaired V7 source-loss cutover, recovered risk-fraud production service, full P2S rebase activation, stale first-cycle candidate supersession, and remaining non-zero cycle boundary. |
| `docs/SYNC_OPS_5C_R1_STRICT_CYCLE_ENVELOPE.md` | SYNC-OPS-5C-R1 strict cycle envelope repair: summary first-nonzero packet rejection, formal publish-and-rebase schema binding, exact S2P/P2S child preservation, approval request, and dry-run boundary. |
| `docs/SYNC_OPS_5C_FINAL_STRICT_CYCLE_CLOSEOUT.md` | SYNC-OPS-5C-FINAL strict nonzero execution: exact cycle approval bundle, one-file S2P backup/apply/test, first-cycle P2S stage/activation, final parity, owner handoff, and non-claims. |
| `docs/AGENT_SYNC_ONE_COMMAND_WORKFLOW.md` | Operator runbook for future one-command agent sync cycles: experiment fork, change units, cycle plan, machine approval, execution, status, recovery, and blockers. |
| `docs/CODEX_AGENT_SYNC_OPERATOR_WORKFLOW.md` | Short future Codex prompt and boundaries for generating approved cycle plans and executing exact approval bundles. |
...
| `docs/L4_RUNTIME_REVIEW_EVIDENCE_R8_13O.md` | R8-13O/R8-13P L4 runtime review evidence package and phase preflight, retained with the R8-13Q post-approval compute-default runtime status. |
...
and documentation cleanup guidance without changing runtime behavior.
...
L3 composite LLM use, if added later, is limited to default-off language
explanation and conflict summarization; deterministic fusion remains the source
of stance, gate, risk score, weights, and confidence.
CS1-C1X adds the current health identity migration policy and request-as-of
temporal guard. It also records scoped production wrapper patches and L4
compute-only listener recovery in `docs/CS1_C1X_ACCELERATED_BULK_REMEDIATION.md`.
The phase keeps compute identity strict, does not call external invoke, does
not change runtime bindings, and does not promote non-L4 services into default
runtime.
CS1-C3X adds formal, dimension-scoped L3 member validation for macro composite
outputs, removes service-local macro stand-ins from public formal evidence, and
