...
from react_agent.ops_sync.sync_5a_r2x import (
...
from react_agent.ops_sync.sync_5a_r3x import (
...
from react_agent.ops_sync.sync_5a_r4x import (
...
from react_agent.ops_sync.sync_5a_r5x import (
    build_real_readonly_preflight,
    validate_source_loss_recovery_plan_v5,
)
from react_agent.ops_sync.sync_5a_r6x import (
    build_real_readonly_preflight_v6,
    build_v6_projection_bundle,
...
from react_agent.ops_sync.sync_5a_r7x import (
    build_real_readonly_preflight_v7,
    validate_source_loss_recovery_plan_v6_for_v7,
    validate_source_loss_recovery_plan_v7,
)
from react_agent.ops_sync.sync_approval import load_approval, validate_approval
from react_agent.ops_sync.sync_artifacts import artifact_store_preflight
from react_agent.ops_sync.sync_bootstrap import (
...
from react_agent.ops_sync.sync_coverage import load_plan_and_build_ledgers
from react_agent.ops_sync.sync_cycle import (
...
from react_agent.ops_sync.sync_environment import build_environment_snapshot
...
from react_agent.ops_sync.sync_p2s import (
    p2s_activate,
    p2s_recover,
    p2s_rollback,
    p2s_stage,
...
from react_agent.ops_sync.sync_plan import (
...
from react_agent.ops_sync.sync_process_launcher import (
...
from react_agent.ops_sync.sync_registry import load_sync_policy, validate_static_registry
from react_agent.ops_sync.sync_s2p import (
...
from react_agent.ops_sync.sync_summary import p2s_summary_from_plan
...
def cmd_cycle_publish_and_rebase(args: argparse.Namespace) -> int:
    ...
    if not args.execute:
        ...
        if approval.get("schema_version") == "agent_sync_cycle_approval_request_v1":
            ...
            from react_agent.ops_sync.sync_cycle import validate_cycle_approval_bundle
...
def cmd_source_loss_plan(args: argparse.Namespace) -> int:
    ...
    qualification = read_json(Path(args.qualification_evidence)) if getattr(args, "qualification_evidence", "") else {
        "schema_version": "agent_sync_cli_missing_qualification_evidence_v1",
        "canonical_sha256": "",
        "full_materialization": {"valid": False},
        "offline_nonmutation": {"valid": False},
    }
    bundle = build_v6_projection_bundle(
        final_head=args.final_head,
        precutover_closeout=closeout,
        runtime_identity=runtime,
        qualification_evidence=qualification,
    )
    recovery = bundle["source_loss_recovery_plan_v6"]
    validation = validate_source_loss_recovery_plan_v6(recovery)
    request = bundle["source_loss_cutover_approval_request_v6"]
    request_validation = validate_source_loss_cutover_approval_request_v6(request, recovery)
    payload = {
        "summary_title": "agent-sync source-loss plan",
        **bundle,
        "source_loss_recovery_plan_v6_validation": validation,
        "source_loss_cutover_approval_request_v6_validation": request_validation,
...
def cmd_source_loss_explain(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "file_action_metadata_complete": not any(
            key not in action
            for action in plan.get("file_actions") or []
            if isinstance(action, dict)
            for key in (
                "source_file_type",
                "source_mode",
        ...
        "offline_validation_nonmutating": bool(
            _as_mapping(_as_mapping(plan.get("qualification_evidence")).get("offline_nonmutation")).get("valid")
        ),
        "irreversible_boundary": "source-loss cutover requires exact machine approval",
    ...
    if plan.get("schema_version") == "agent_sync_source_loss_recovery_plan_v5":
        ...
        if not validation["valid"]:
            payload = {
                ...
                "reason": "fresh_candidate_projection_invalid",
        ...
        if not validation["valid"]:
            payload = {
                "schema_version": "agent_sync_cli_error_v1",
                "summary_title": "agent-sync source-loss execute",
                "reason": "source_loss_plan_invalid",
                "validation": validation,
                "exit_code": 7,
            }
            return print_or_json(args, payload, ["reason=source_loss_plan_invalid"])
...
def build_parser() -> argparse.ArgumentParser:
    ...
    source_loss_plan.add_argument("--qualification-evidence", default="")
